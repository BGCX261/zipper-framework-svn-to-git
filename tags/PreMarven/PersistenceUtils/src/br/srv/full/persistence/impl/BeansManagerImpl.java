 ...
 * 
 */
public class BeansManagerImpl extends BeansManagerAbstract {
	...
	public <bean, key> bean find(Class<bean> _beanClass, key _key) {
		...
		bean l_result = (bean) l_em.find(_beanClass, _key);
	...
	public <bean, params> Object find(Class<bean> p_class, QueryID p_query, Map<String, Object> p_params) {
		...
		return getResult(p_class, l_em, l_query);
	}
	...
	public <bean> Object find(Class<bean> p_class) {
		...
		Query l_query = getQueryIdAllBeans(l_em, p_class);
	...
	public <bean> bean persist(bean _bean) throws PersistBeansException {
		...
		try {
			...
			if (l_cause instanceof PropertyValueException) {
				PropertyValueException l_e = (PropertyValueException) l_cause;
				log.warn("problema ao persistir o objeto: " + _bean.getClass() + " com o conteúdo: " + _bean);
				log.warn("Propriedade com problema: " + l_e.getPropertyName());
				throw new BeansManagerPropertyException(e, l_e.getPropertyName());
			} else
	...
	protected EntityManager createEntityManager(boolean... p_tx) {
		EntityManager l_em = getEmf().createEntityManager();
