...
public abstract class ConfigManager {
	...
	/**
	 * libera ConfigManager. Isto não garante que o GC vá coleta-lo, pois pode
	 * estar sendo usado por outro objeto. este objeto deverá receber uma
	 * exceção ao tentar usar este ConfigManager depois dele ter sido
	 * dispensado.
	 * 
	 */
	public void dispose() {
	...
	public <T> T getInstance(Class<T> p_class) {
		...
		if (dispose)
			throw new InvalidStateException(
					"Instancia do configmanager foi invalida, foi solicitada que ela seja dispensada!");
	...
	public <T> T getClassImplentation(String p_prefix, Class<?> p_class) throws ClassNotFoundException {
		T l_classImplentation = getClassImplentation(p_prefix, p_class.getName());
		if (dispose)
			throw new InvalidStateException(
					"Instancia do configmanager foi invalida, foi solicitada que ela seja dispensada!");
		return l_classImplentation;
	}
	...
	public <T> T getClassImplentation(String p_prefix, String p_class) throws ClassNotFoundException {
		...
		if (l_strClassName == null) {
			...
			// throw new ClassNotFoundException("Não encontramos implementação
			// para " + p_class);
		}
		...
		try {
			...
			log.warn("Problemas ao tentar recuperar a classe!", e);
			// TODO ANALISAR A CRIAÇÃO DE UMA EXCEPTION ESPECIFICA DO
			// ConfigManager
			throw e;
		}
	}
