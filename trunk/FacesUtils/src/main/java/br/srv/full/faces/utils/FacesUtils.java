...
public class FacesUtils {
	 ...
	 * @param p_component
	 */
	public static void notNull(String p_string, Object p_component) {
		if (p_component == null) {
			throw new NullPointerException(FacesMessageUtils.getExceptionMessageString(
					FacesMessageUtils.NULL_PARAMETERS_ERROR_MESSAGE_ID, p_string));
		}
	}
	...
	/**
	 * <p>
	 * PRECONDITION: argument <code>response</code> is non-null and has a
	 * method called <code>getContentType</code> that takes no arguments and
	 * returns a String, with no side-effects.
	 * </p>
	 * 
	 * <p>
	 * This method allows us to get the contentType in both the servlet and
	 * portlet cases, without introducing a compile-time dependency on the
	 * portlet api.
	 * </p>
	 * 
	 * @param response
		...
		if (null != response) {
			...
			try {
				Method method = ReflectionUtils.lookupMethod(response.getClass(), "getContentType",
						VirtualConstants.EMPTY_CLASS_ARGS);
				if (null != method) {
					Object obj = method.invoke(response, VirtualConstants.EMPTY_METH_ARGS);
					if (null != obj) {
						result = obj.toString();
					}
				}
	...
	public static FeatureDescriptor getFeatureDescriptor(String name, String displayName, String desc, boolean expert,
